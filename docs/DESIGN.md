...
每工具一个 `templates/configs/<tool>.lua` 模块（编进二进制），导出 `M.render(cfg, ctx) → string` + `M.target_path(cfg, ctx) → path`。详例见 §9.9 inline 注册形态——**机制与 §9.9 的 `register_renderer` 完全一致**：内置走 apply-start 注册（commands/blueprint.cpp 顶端的 `kBuiltinRenderers` 表，preload 时跑 `lua_frontend::wrap_embedded_module`），blueprint 自带 renderer 走 apply 时注册，同一注册表，无双码路径。已兑现 §24.1 AI；`config_renderer.cpp` 是纯 callback dispatch，零 lua C API（不变量 9）。
...
| **M** | 扩展机制交付时间线 | **v1.0 必交**——blueprint 模型完整性的一部分；不留 v1.x。vcpkg bootstrap / emscripten config 这两个 component.cpp 硬编码特例必须借此机制收编 | **全部落地 2026-05-07 (v0.4.x)**：（a）`post_install` hook 已落地（`[tool.X.post_install]` 解析 + apply 阶段路径穿越守卫 + cmd /c / sh 包装 + cache 命中跳过；v0.2.x 加 `bp:` 前缀走 bp_source_root）；（b）vcpkg bootstrap 特例已通过 source-zip fallback + post_install 收编进 `main/cpp-toolchain`，无 C++ 硬编码；（c）C++ 侧 resolver hook 仍以 per-scheme TU 为模板（github / pwsh-module）；（d）**Lua-side `luban.register_renderer` (v0.4.0) + `register_resolver` (v0.4.2) 已实现**——Engine + 两个 Registry (renderer/resolver) 一起在 commands/blueprint.cpp run_apply 顶层构造，跨 parse / lock-resolve / render 全过程；Lua bp 第二次 parse_in_engine 跑 register_* 副作用；config_renderer::render_with_registry / source_resolver::resolve_with_registry 优先查 registry 再 fall through。原计划提到的 "program_renderer 单 Engine 寿命问题" 通过两次 parse + 长寿命 Engine 的方式解决，没有 refactor 内置 renderer 路径（同一注册表 / "无双码路径" 是 v0.4.x cosmetic 收尾）。tests: `tests/test_lua_register.cpp` 12 cases 覆盖 register / find / shadow / error 路径。 |
...
| **P** | "Program" 名字别扭 + L2/L3 关系隐式 | **改名（单数）+ 显式绑定**：`[programs.X]` → `[config.X]`、`[tools.X]` → `[tool.X]`、`[files."path"]` → `[file."path"]`（TOML 习惯单数）；模块 `program_renderer` → `config_renderer`；目录 `templates/programs/` → `templates/configs/`。新增可选字段 `[config.X] for_tool = "Y"` 显式声明该 config 配的是哪个 tool（缺省 = X，schema 化此前隐式的同名约定）| **代码全部落地 v0.2.0 (2026-05-06)**：blueprint_toml + blueprint_lua 解析切单数 + 拒绝旧复数 key；`ProgramSpec` → `ConfigSpec`、`.programs` → `.configs`、`find_program` → `find_config`；`src/program_renderer.{cpp,hpp}` git-rename 到 `config_renderer.*`、`templates/programs/<X>.lua` 到 `templates/configs/<X>.lua`、5 个内置 renderer 路径同步 + CMakeLists embed_text foreach 改 `EMBED_CONFIGS_DIR` / `embedded_configs::`；`for_tool` 字段在 TOML + Lua 两边解析，apply 时 `cfg.for_tool.value_or(cfg.name)` 选 renderer。Breaking — luban-bps 三张图同 PR 切；旧 plural key 进 `unknown section` 路径，不留 alias。 |
...
| **AH** | Core ↔ Lua 解耦边界 | core 模块（`renderer_registry` / `resolver_registry` / `config_renderer` / `source_resolver` / `blueprint_apply`）禁包 `lua.h` / `lauxlib.h` / 引 `lua_State`；registry Entry 字段是 `std::function<...>`，frontend 来源（Lua bp / 内置 builtin / 未来 native plugin / JS）对 core 透明。所有 frontend → core 的桥梁集中在 `src/lua_frontend.cpp` 一个 TU——它是项目里唯一允许 include lua C 头的边界文件。`shared_ptr<LuaRef>` 模式管 ref 寿命，debug-only `StackGuard` RAII 守 lua C API 栈平衡。 | **落地 2026-05-07**：commits 5b7390b（docs anchor）→ 3b7ae51（lua_frontend + register_native）→ b570978（dispatch 切到 callback）→ 6c15055（ApplyOptions 撤 Engine 字段）→ 82d63e0（删 register_lua + Entry-with-lua_State*）→ 7b54f0c（builtin 进 registry，render_with_source 删）。grep `lua\.h\|lauxlib\.h\|lua_State` core 5 文件零命中。 |
| **AI** | Frontend 主次 / TOML 二等 | 内部模型：**Lua 是 primary frontend**（parse + 完整 register_* 能力）；**TOML 是 degenerate frontend**（只产数据，不能 register_*）。两者 fold 到同一 `BlueprintSpec`；apply pipeline 永远走 Lua engine + 统一 registry dispatch，5 个内置 renderer 也注册成 `std::function`（兑现 §9.9 line 656 "无双码路径"承诺）。**用户感知不变**——纯 TOML bp 仍可零 Lua 跑通；架构上 TOML 是 Lua frontend 的 ability-restricted projection，apply 时仍构造 Lua engine（sub-ms 成本）只是不跑 register_* 副作用。 | **落地 2026-05-07**：commit 6c15055（commands/blueprint.cpp::run_apply 永远构造 apply_engine + 两 registries，TOML bp 也走）+ 7b54f0c（preload_builtin_renderers 把 5 个内置进 registry，dispatch 单码路径）。 |
