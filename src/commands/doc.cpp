...
//      luban-installed doxygen on PATH (`luban setup --with doxygen` puts
//      one there). Output lands at `<project>/build/doc/html/index.html`.
//
// `--open` opens the generated index in the user's default browser.
// `--clean` deletes `<project>/build/doc/` and exits.
//
// Doxygen itself is NOT bundled with luban — the user runs `luban setup
// --with doxygen` first (or installs it manually). We surface a clear
// error pointing there if doxygen isn't on PATH.
...
int run_doc(const cli::ParsedArgs& a) {
    ...
    // a `luban setup --with doxygen`-installed binary takes precedence.
    ...
    if (rc != 0) {
        ...
        log::info("not on PATH? run `luban setup --with doxygen` and retry.");
        return rc;
    }
...
void register_doc() {
    ...
    c.long_help =
        ...
        "\n"
        "  Doxygen is not bundled with luban. Install once via:\n"
