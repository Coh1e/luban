...
std::expected<InstallReport, Error> install(const std::string& name, bool force,
    ...
    // 8c. emscripten special-case: write `.emscripten` config so emcc/emcmake
    //     can locate LLVM_ROOT, BINARYEN_ROOT, EMSCRIPTEN_ROOT, NODE_JS without
    //     needing emsdk activation. Looks up node from registry; skipped if
    //     node is missing (emcmake will then surface the error at build time).
    if (name == "emscripten") {
        fs::path config = final / "emscripten" / ".emscripten";
