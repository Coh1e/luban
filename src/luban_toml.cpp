...
void load_from_table(const ::toml::table& tbl, Config& out) {
    ...
    // [ports.X] — per-project vcpkg port → cmake target hints (议题 AB).
    // Each entry is a sub-table with `find_package` (string) and `targets`
    // (string array). Either may be omitted; render time fills the gap.
    if (auto* ports = tbl["ports"].as_table()) {
        for (auto&& [key, node] : *ports) {
            auto* sub = node.as_table();
            if (!sub) continue;  // [ports.X] = "scalar" is a user typo; skip
            PortHint hint;
            if (auto v = (*sub)["find_package"].value<std::string>()) {
                hint.find_package = *v;
            }
            if (auto* arr = (*sub)["targets"].as_array()) {
                for (auto& el : *arr) {
                    if (auto s = el.value<std::string>()) {
                        hint.targets.push_back(*s);
                    }
                }
            }
            out.ports[std::string(key.str())] = std::move(hint);
        }
    }
}
