...
std::vector<std::pair<std::string, std::string>> env_dict() {
    // VCPKG_ROOT must be set if vcpkg is installed — CMakePresets.json
    // generated by `luban new` references $env{VCPKG_ROOT}/scripts/...
    // and child cmake processes need it. Was missing here previously
    // (only set in HKCU by env --user); luban-spawned cmake without
    // env --user couldn't find vcpkg. This fixes that.
    //
    // VCPKG_* cache vars route vcpkg's own caches into <cache>/vcpkg/*
    // instead of vcpkg's defaults (<vcpkg_root>/downloads scattered around).
    // Keeps everything XDG-respecting; one place to clear caches.
