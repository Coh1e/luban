...
// 议题 AB: [ports.X] per-project vcpkg port → cmake target hints.

TEST_CASE("luban_toml: empty config has no port hints") {
    auto cfg = luban::luban_toml::load_from_text("");
    CHECK(cfg.ports.empty());
}

TEST_CASE("luban_toml: [ports.X] full hint (find_package + targets)") {
    auto cfg = luban::luban_toml::load_from_text(
        "[ports.mylib]\n"
        "find_package = \"MyLib\"\n"
        "targets = [\"MyLib::core\", \"MyLib::extra\"]\n");
    REQUIRE(cfg.ports.count("mylib") == 1);
    auto& h = cfg.ports.at("mylib");
    CHECK(h.find_package == "MyLib");
    REQUIRE(h.targets.size() == 2);
    CHECK(h.targets[0] == "MyLib::core");
    CHECK(h.targets[1] == "MyLib::extra");
}

TEST_CASE("luban_toml: [ports.X] partial hint (targets only) parses") {
    auto cfg = luban::luban_toml::load_from_text(
        "[ports.barelink]\ntargets = [\"bare::lib\"]\n");
    REQUIRE(cfg.ports.count("barelink") == 1);
    CHECK(cfg.ports.at("barelink").find_package.empty());
    REQUIRE(cfg.ports.at("barelink").targets.size() == 1);
    CHECK(cfg.ports.at("barelink").targets[0] == "bare::lib");
}

TEST_CASE("luban_toml: [ports.X] = scalar (typo) is skipped silently") {
    auto cfg = luban::luban_toml::load_from_text(
        "[ports]\nbroken = \"oops\"\n");
    CHECK(cfg.ports.empty());
}
